(async () => {
  ...
  const els = {
    ...
    wordGrid: $('wordGrid'), emptyState: $('emptyState'), emptyTitle: $('emptyTitle'), emptyText: $('emptyText'), meaningToggle: $('meaningToggle'), continueBtn: $('continueBtn'), footerRange: $('footerRange'), themeToggle: $('themeToggle'), themeLabel: $('themeLabel')
  };
  ...
  function applyTheme(theme) {
    ...
    if (els.themeLabel) els.themeLabel.textContent = theme === 'dark' ? '浅色' : '深色';
    if (els.themeToggle) els.themeToggle.setAttribute('aria-label', theme === 'dark' ? '切换到浅色主题' : '切换到深色主题');
  }
  ...
  function renderDay() {
    ...
    els.daySummary.textContent = day.type === '休息'
      ...
      : `今天需要学习 ${day.count} 个新词。完成一个，就让进度向前一点。`;
    renderStrip();
    renderProgress();
    renderWords();
    updateNav();
      ...
      <article class="word-card ${isLearned ? 'learned' : ''}" data-id="${w.id}">
        <div class="card-top">
          <div>
            ...
            <div class="word-main"><h3>${escapeHTML(w.word)}</h3><span class="pos">${escapeHTML(w.pos)}</span></div>
          </div>
        ...
        <div class="card-meta">
          <span class="badge level-${w.levelCode}">${escapeHTML(w.level)}</span>
